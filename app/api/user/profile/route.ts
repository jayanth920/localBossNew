import { NextRequest, NextResponse } from 'next/server'
 ...
 export async function PATCH(req: NextRequest) {
   try {
     ...
     // Parse the JSON body
     const { userId, password, phone, profilePic } = await req.json()
     ...
     if (profilePic && profilePic !== '' && profilePic !== null && user.profilePic !== profilePic) {
       const uniqueName = `${user.username}_${uuidv4()}_${Date.now()}${getImageExtension(profilePic)}` // Get dynamic file extension
 
       // Decode the Base64 string and create a buffer
