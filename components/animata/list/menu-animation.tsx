...
export default function MenuAnimation({ menuItems }: MenuAnimationProps) {
  ...
  return (
    <div className="flex min-w-fit flex-row md:flex-col gap-2 md:gap-3 md:overflow-hidden pl-6 sm:px-10 absolute top-[2vh] md:top-[5vh] left-[3vw] md:left-0">
      {menuItems.map((item, index) => (
        <button key={index} onClick={() => router.push(`/${item.toLowerCase().replace(/\s+/g, '-')}`)} className="group flex items-center md:gap-1 gap-2 cursor-pointer min-h-fit">
          ...
          <p className="z-10 -translate-x-6 font-mono font-semibold text-amber-50 transition-transform duration-300 ease-out group-hover:translate-x-0 group-hover:text-blue-500 md:-translate-x-12 text-md md:text-3xl md:group-hover:translate-x-0 leading-none ">
            {item}
          </p>
        </button>
      ))}
