    ...
    <div className="flex min-w-fit flex-col gap-3 overflow-hidden px-10 absolute top-[5vh] left-0">
      {menuItems.map((item, index) => (
        <button key={index} onClick={() => router.push(`/${item.toLowerCase().replace(/\s+/g, '-')}`)} className="group flex items-center gap-1 cursor-pointer min-h-fit">
          ...
          <p className="z-10 -translate-x-6 font-mono font-semibold text-black transition-transform duration-300 ease-out group-hover:translate-x-0 group-hover:text-blue-500 dark:text-white md:-translate-x-12 md:text-3xl md:group-hover:translate-x-0 leading-none ">
            {item}
          </p>
        </button>
      ))}
